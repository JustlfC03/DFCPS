...
from dataloader_eval import VOC
from dataloader_eval import ValPre
...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device):
        ...
        self.save_path = '/home/lmz/lmz/TorchSemiSeg-main/TorchSemiSeg-main/exp.voc/voc8.res50v3+.CPS/results2/results'

        print(
            np.array(img).shape[0],
            np.array(img).shape[0], pred.shape[0], pred.shape[1])
        ...
        pred = cv2.resize(pred, (orininal_w, orininal_h),
                          interpolation=cv2.INTER_LINEAR)

        seg_img = np.reshape(
            np.array((128, 0, 0), np.uint8)[np.reshape(pred, [-1])],
            [orininal_h, orininal_w, -1])
        seg_img = np.squeeze(seg_img, axis=2)
        #print(seg_img.shape)
        #------------------------------------------------#
        #   将新图片转换成Image的形式
        #------------------------------------------------#
        image = Image.fromarray(np.uint8(seg_img)).convert('RGB')
        ...
        self.save_path = self.save_path = '/home/lmz/lmz/TorchSemiSeg-main/TorchSemiSeg-main/exp.voc/voc8.res50v3+.CPS/results2/results_mix'

        image.save(os.path.join(self.save_path, fn))
    ...
    def compute_metric(self, results):
        ...
        result_line = print_iou(iu, mean_pixel_acc, dataset.get_class_names(),
                                True)
        if azure:
