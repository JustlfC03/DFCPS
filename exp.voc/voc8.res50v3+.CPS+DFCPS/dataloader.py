...
def adjust_brightness_contrast(image):
    brightness_range = [0.5, 1.5]  # 亮度范围（0.5-1.5）
    contrast_range = [0.0, 3.0]  # 对比度调整值（0.0-3.0）
    # 随机生成亮度的值
    ...
    # 应用亮度和对比度调整
    adjusted_image = cv2.addWeighted(image, contrast, np.zeros(image.shape, image.dtype), 0, brightness - 128)
...
class TrainPre_strong(object):
    ...
    def __call__(self, img, gt=None):
        ...
        p_img = adjust_brightness_contrast(p_img)
        p_img = random_gaussian_blur(p_img)
...
def get_train_loader(engine, dataset, train_source, unsupervised=False):
    ...
    train_preprocess_strong = TrainPre_strong(config.image_mean, config.image_std)
    train_preprocess_weak = TrainPre_weak(config.image_mean, config.image_std)

    if 'selftrain' in train_source:
        train_dataset = dataset(data_setting, "train", train_preprocess_strong, train_preprocess_weak, config.tot_samples, unsupervised=unsupervised)
    else:
        train_dataset = dataset(data_setting, "train", train_preprocess_strong, train_preprocess_weak,
...
class VOC(BaseDataset):
    def __init__(self, setting, split_name, preprocess_strong=None, preprocess_weak=None,
                 file_length=None, training=True, unsupervised=False, pseudo_label=False):
        self.istraining = training
        self.unsupervised = unsupervised
        super(VOC, self).__init__(setting, split_name, file_length)
        ...
        self.preprocess_strong = preprocess_strong
        self.preprocess_weak = preprocess_weak
    ...
    def __getitem__(self, index):
        ...
        img_o = img[:, :, ::-1]
        gt_o = gt
        if self.preprocess_strong is not None:
            img_strong, gt_strong, extra_dict_strong = self.preprocess_strong(img_o, gt_o)

        if self.preprocess_weak is not None:
            img_weak, gt_weak, extra_dict_weak = self.preprocess_weak(img_o, gt_o)

        if self._split_name in ['train', 'trainval', 'train_aug', 'trainval_aug']:
            img_strong = torch.from_numpy(np.ascontiguousarray(img_strong)).float()
            if gt_strong is not None:
                gt_strong = torch.from_numpy(np.ascontiguousarray(gt_strong)).long()

            
            img_weak = torch.from_numpy(np.ascontiguousarray(img_weak)).float()
            if gt_weak is not None:
                gt_weak = torch.from_numpy(np.ascontiguousarray(gt_weak)).long()
            # if self.preprocess_strong is not None and extra_dict_strong is not None:
            #     for k, v in extra_dict_strong.items():
        ...
        output_dict = dict(data=[img_strong, img_weak], fn=str(names),
                           n=len(self._file_names))
        extra_dict = {}
        if gt is not None:
            extra_dict['label'] = [gt_strong, gt_weak]

        if self.preprocess_strong is not None and extra_dict is not None:
    ...
    def get_class_names(*args):
        return ['background', 'Intestinal Polyp']
